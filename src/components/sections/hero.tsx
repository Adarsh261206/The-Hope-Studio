...
const bgImage = "https://images.unsplash.com/photo-1755464903950-700dc907db74?q=80&w=2070&auto=format&fit=crop"
...
export function HeroSection() {
  ...
  return (
    <section
      ref={ref}
      className="relative h-screen flex flex-col bg-cream overflow-hidden"
    >
      <motion.div
        className="absolute inset-0 bg-cover bg-[center_30%] sm:bg-center"
        style={{ y: bgY, backgroundImage: `url('${bgImage}')`, willChange: "transform" }}
      />

      <div
        className="absolute inset-0 pointer-events-none z-[2]"
        style={{
          background: "linear-gradient(180deg, rgba(249,246,243,0.55) 0%, rgba(249,246,243,0.20) 25%, rgba(249,246,243,0.05) 45%, rgba(249,246,243,0.15) 65%, rgba(249,246,243,0.45) 100%)"
        }}
      />
      ...
      <div className="absolute inset-0 pointer-events-none z-[3] max-sm:hidden">
        <div className="container-main h-full relative">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="absolute top-0 bottom-0 w-[1px] bg-deep/8"
              style={{ left: `${(i / 5) * 100}%` }}
            />
          ))}
        </div>
      ...
      <div className="relative z-[4] flex-1 flex flex-col justify-center container-main pt-16 sm:pt-[7.5rem] pb-28 sm:pb-[18rem]">
        <div className="w-full max-w-2xl">
          <motion.h1
            ...
            className="text-[3rem] sm:text-[2.75rem] md:text-[3rem] lg:text-[4rem] xl:text-[5.125rem] leading-[1.05] sm:leading-[1] font-serif font-medium text-deep text-balance"
          >
            Wellness That
            <br />
          ...
          >
            <a
              ...
              className="group inline-flex items-center gap-2.5 sm:gap-3 px-5 py-3 sm:px-6 sm:py-3 bg-deep text-cream rounded-full text-sm font-sans font-medium hover:bg-deep-800 transition-all duration-300 hover-scale"
            >
              Book Consultation
              <span className="w-8 h-8 sm:w-[2.125rem] sm:h-[2.125rem] rounded-full bg-cream text-deep flex items-center justify-center transition-all duration-300 group-hover:bg-white/90 group-hover:rotate-12">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-2 px-5 py-3 sm:px-6 sm:py-3 bg-deep/10 backdrop-blur-lg text-deep border border-deep/20 rounded-full text-sm font-sans font-medium hover:bg-deep/20 transition-all duration-300 hover-scale"
            >
              Enquire Now
            </a>
          </motion.div>
          ...
          >
            {[
              ...
              <div
                key={stat.label}
                className="inline-flex items-center gap-2 px-3.5 py-2 sm:px-4 sm:py-2.5 bg-cream/30 backdrop-blur-sm border border-cream-dark/40 rounded-full transition-all duration-300 hover:bg-cream/50"
              >
                <span className="text-sm sm:text-base font-serif font-medium text-deep">{stat.value}</span>
                <span className="text-[0.55rem] sm:text-[0.6rem] text-deep/60 font-sans uppercase tracking-wider">{stat.label}</span>
              </div>
            ))}
          </motion.div>
        </div>
      ...
      >
        <div className="flex items-center gap-5 sm:gap-6 md:gap-7 animate-scroll-left pointer-events-auto gpu-layer">
              ...
              <div key={i} className={`flex-shrink-0 ${floatClass}`}>
                <div className="relative w-[10rem] h-[12.5rem] sm:w-[11rem] sm:h-[13rem] md:w-[11.5rem] md:h-[13.5rem] rounded-full overflow-hidden group cursor-pointer shadow-[0_8px_24px_rgba(0,0,0,0.18)] sm:shadow-[0_12px_36px_rgba(0,0,0,0.22)] transition-shadow duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.28)]">
                  <div
                    ...
                    style={{ backgroundImage: `url('${img}')`, willChange: "transform" }}
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-deep/70 via-deep/15 to-transparent sm:from-deep/70 sm:via-deep/15 sm:to-transparent" />
                  <div className="absolute bottom-4 sm:bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 sm:gap-2.5 md:gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-400">
                    <span className="w-2.5 h-[1.5px] sm:w-3 bg-cream/80" />
                    <span className="text-[0.55rem] sm:text-[0.55rem] text-cream/90 font-sans font-medium uppercase tracking-widest whitespace-nowrap">
